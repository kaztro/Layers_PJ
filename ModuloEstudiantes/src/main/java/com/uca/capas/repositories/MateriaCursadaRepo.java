...
public interface MateriaCursadaRepo extends JpaRepository<MateriaCursada, Integer> {
	...
	"FROM( "+
		...
		"FROM( "+
			...
			"FULL OUTER JOIN public.estudiante AS e "+
			"ON mc.id_estudiante = e.id_estudiante "+
			"WHERE mc.nota < 6 AND mc.nota >0 "+
			"GROUP BY mc.id_estudiante "+
		") AS fir "+
		"FULL OUTER JOIN public.materia_cursada AS cru "+
		"ON fir.id_estudiante = cru.id_estudiante "+
		"FULL OUTER JOIN public.estudiante AS es "+
		"ON fir.id_estudiante = es.id_estudiante "+
		"WHERE cru.nota >= 6 AND cru.nota <=10 "+
		"GROUP BY cru.id_estudiante, fir.Reprobadas "+
	") AS ma "+
	"FULL OUTER JOIN public.materia_cursada AS mc "+
	"ON ma.id_estudiante = mc.id_estudiante "+
	"FULL OUTER JOIN public.estudiante AS e "+
	"ON mc.id_estudiante = e.id_estudiante "+
